...
def convert_usd(amount, target_currency, exchange_rates):
    """Convert an amount in USD to a specified currency based on exchange rates."""
    if target_currency in exchange_rates:
        return amount * exchange_rates[target_currency]
    else:
        print(f"Error: Exchange rate for {target_currency} not available.")
        return None

def main():
    """Main function to get user input and perform currency conversion."""
    ...
    try:
        ...
        print("Invalid input! Please enter a valid number for USD amount.")
